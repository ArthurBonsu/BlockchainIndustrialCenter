...
// Deployed on Sepolia — block 10335207 (VehicleRegistry) & 10335219 (RevocationManager)
const VEHICLE_REGISTRY_ADDRESS = '0xF819ca156ac0ACE5fe1d7D93A51887AeDb5a983B';
const REVOCATION_MANAGER_ADDRESS = '0xb364D96ba9Cd44B77eDE987Ad96794501985FdC6';
...
class RevChainTest {
    constructor() {
        ...
        if (!process.env.PRIVATE_KEY) {
            throw new Error('PRIVATE_KEY is not set in .env file. Cannot initialize wallet.');
        }
        if (!process.env.INFURA_PROJECT_ID) {
            throw new Error('INFURA_PROJECT_ID is not set in .env file.');
        }
    ...
    async testRegistration(numVehicles = 5) {
        ...
        for (let i = 0; i < numVehicles; i++) {
            ...
            try {
                // ✅ NEW CONTRACT: no need to clear existing vehicle before each registration
                // Multiple unique pseudonyms can be registered from the same address
    ...
    async testRevocation(numTests = 3) {
        ...
        if (this.account2) {
            ...
            console.log(`\n   💡 Single-wallet mode: new contract allows multiple pseudonyms per address`);
        }
        ...
        for (let i = 0; i < numTests; i++) {
            ...
            try {
                ...
                // ✅ NEW CONTRACT: no one-vehicle-per-address restriction
                // Each unique pseudonym can be registered independently from any wallet
        ...
        // ✅ NEW CONTRACT: reporter is still registered (no clearing was needed)
        // No re-registration step required in single-wallet mode
