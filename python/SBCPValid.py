        ...
        for i in range(num_transactions):
            ...
            for response in validator_responses:
                if not isinstance(response, Exception):
                    ...
                    tier = response["finality_tier"]
                    if tier in finality_achievements:
                        finality_achievements[tier] += 1
            
            results["transactions"].append({
                "tx_id": tx_data["tx_id"],
                "validator_responses": successful_validations,  # Store actual responses, not count
                "validator_response_count": len(successful_validations),
