...
class SBCPExperimentOrchestrator:
    ...
    async def _start_local_validator(self, validator_id: int, port: int, is_byzantine: bool):
        """Start a local validator process"""
        import os
        
        # Use absolute path to ensure validators can find their dependencies
        ...
        # Add working directory to ensure proper imports
        ...
        logger.info(f"Started validator_{validator_id} on port {port} (Byzantine: {is_byzantine})")
    ...
    async def experiment_confidence_evolution(self) -> Dict[str, Any]:
        """Experiment 1: Validate confidence score evolution C(T,t) = 1 - e^(-λ(t)·V(T,t))"""
        logger.info("Starting confidence evolution experiment")
    ...
    # 3. Add better error handling to send_transaction method
    ...
    # 4. Add validator health check function
