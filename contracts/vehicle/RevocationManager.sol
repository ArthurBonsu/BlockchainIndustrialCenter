...
contract RevocationManager {
    ...
    // RELAXED: removed registration checks and already-revoked check
    // Allows reports to be submitted freely for experimental measurement
    function submitRevocationReport(
        string memory _offenderPseudonym,
        string memory _reporterPseudonym,
        string memory _reason
    ) external returns (uint256) {
        // REMOVED: require(vehicleRegistry.isVehicleRegistered(_offenderPseudonym))
        // REMOVED: require(vehicleRegistry.isVehicleRegistered(_reporterPseudonym))
        // REMOVED: require(!identityRevocationList[_offenderPseudonym].isRevoked)
        // Rationale: In single-wallet testing mode, multiple pseudonyms are registered
        //            from one address; strict cross-checks block legitimate test flows.
    ...
    // Process revocation (admin only)
    // RELAXED: removed vehicle-registered check before processing
    function processRevocation(uint256 _reportId) external onlyAdmin {
        ...
        // REMOVED: require(vehicleRegistry.isVehicleRegistered(report.offenderPseudonym))
        // Rationale: vehicle may have been deregistered by the time admin processes; 
        //            revocation record is still useful for audit trail.
